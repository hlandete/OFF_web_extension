...
const checkFilterCondition = (
  ...
  const productFilterData = mapProductFilterData(product)

  console.log(productFilterData)
  const matchers: Record<string, (values: string[]) => boolean> = {
    categories: (values) =>
      values.some((value) => productFilterData.categories.includes(value)),
    labels: (values) =>
      values.some((value) => productFilterData.labels.includes(value)),
    keywords: (values) =>
      values.some((value) => productFilterData.keywords.includes(value)),
    allergens: (values) =>
      !values.some((value) => productFilterData.allergens.includes(value))
  }
  ...
  for (const [filterType, filterValues] of Object.entries(filters)) {
    ...
    validations.push(iterate(filterValues))
  }
    ...
    unprocessedProducts.map(async (data) => {
      ...
      if (product) {
        ...
        return {
          ...
          name: product.product_name,
          brand: product.brands,
          condition: matchFilters // Cambia a 'true' si cumple con los filtros
        }
      }
